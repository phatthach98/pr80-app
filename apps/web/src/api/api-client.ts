...
export class ApiClient {
        ...
        if (error.response?.status === 401 && !originalRequest._retry) {
          ...
          if (refreshToken) {
            try {
              ...
              const refreshResponse = await axios.post(`${this.client.defaults.baseURL}/api/auth/refresh`, {
                refreshToken,
              });
   ...
   */
  private handleApiError(apiError: ApiErrorResponse): { data: null; error: string; success: false } {
